...
def test_transfer_change_is_paired_by_seed_before_mean_and_std() -> None:
    ...
    assert low["per_seed"] == [
        {"seed": 42, "macro_f1_change": pytest.approx(-0.1)},
        {"seed": 43, "macro_f1_change": pytest.approx(0.2)},
        {"seed": 44, "macro_f1_change": pytest.approx(-0.1)},
    ]
...
def test_historical_dry_run_writes_valid_results_and_frozen_package(
    ...
    numeric_columns = set(rows[0]) - {"model", "train_regime", "test_regime"}
